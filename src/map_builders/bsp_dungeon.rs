use super::{ MapBuilder, Map, Rect, apply_room_to_map,
             TileType, Position, spawner, SHOW_MAPGEN };
...
impl BspDungeonBuilder {
    ...
    /// Given an input rectangle, produces a randomly sized rectangle no less than 3 in size and 10 in either dimension
    ...
    /// Checks if a room can be placed on the map.
    /// Checks two tiles on either side of the rectangle to prevent overlapping, if the room
    /// is out of bounds, or if it would be placed on an existing room.
    ...
    /// Given two points create a path between them.
    /// Creates 90 degree bends, NOT like Bresenham's
