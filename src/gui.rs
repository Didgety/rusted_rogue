...
use specs::prelude::*;
use super::{ CombatStats, Equipped, gamelog::GameLog, Hidden, HungerClock, HungerState,
             InBackpack, Map, Name, Player, Point, Position, rex_assets::RexAssets,
...
pub fn main_menu(gs : &mut State, ctx : &mut Rltk) -> MainMenuResult {
    ...
    ctx.draw_box_double(24, 18, 31, 11, RGB::named(rltk::WHEAT), RGB::named(rltk::BLACK));
    ctx.print_color_centered(20, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), "Rusty Rogue");
    ctx.print_color_centered(22, RGB::named(rltk::CYAN), RGB::named(rltk::BLACK), "by Jai Veilleux");
    ctx.print_color_centered(24, RGB::named(rltk::GRAY), RGB::named(rltk::BLACK), "Use Up/Down Arrows and Enter");
...
fn draw_tooltips(ecs: &World, ctx : &mut Rltk) {
    ...
    let mouse_pos = ctx.mouse_pos();
    if mouse_pos.0 >= map.width || mouse_pos.1 >= map.height { return; } // mouse out of the window, do nothing
    ...
    if !tooltip.is_empty() {
        ...
        width += 3;
        // tooltip faces left or right depending on if the entity is on the left or right half of the map
        if mouse_pos.0 > 40 {
...
// TODO make drop_item_menu and remove_item_menu generic
